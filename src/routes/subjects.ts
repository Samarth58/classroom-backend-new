...
router.post("/", authenticate, requireRole("admin", "teacher"), validateBody(createSubjectSchema), async (req, res) => {
  try {
    ...
    if (error?.code === "23503" || error?.cause?.code === "23503") {
      return res.status(400).json({ error: "Referenced department does not exist" });
    }
...
// Update subject (partial)
router.patch("/:id", authenticate, requireRole("admin", "teacher"), validateBody(updateSubjectSchema), async (req, res) => {
  try {
    const subjectId = Number(req.params.id);
...
// Delete subject (cascades to classes)
router.delete("/:id", authenticate, requireRole("admin", "teacher"), async (req, res) => {
  try {
    ...
    return res.status(200).json({ data: { message: "Subject deleted successfully" } });
  } catch (error: any) {
    ...
    if (isFkViolation) {
      return res.status(400).json({ error: "Cannot delete subject with active dependencies" });
    }
